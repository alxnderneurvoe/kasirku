...
import 'deletegrosir.dart';

class BoxDataGrosir extends StatefulWidget {
  const BoxDataGrosir({super.key});

  @override
  State<BoxDataGrosir> createState() => _BoxDataGrosirState();
}

class _BoxDataGrosirState extends State<BoxDataGrosir> {
                    ...
                    return Expanded(
                        child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              var hargaGrosir = booking['harga_grosir'];
                              ...
                              if (hargaGrosir is String) {
                                try {
                                  hargaGrosir = double.parse(hargaGrosir);
                                } catch (e) {
                                  print('Error parsing hargaGrosir: $e');
                                  hargaGrosir = 0.0;
                                }
                              } else if (hargaGrosir is! double) {
                                hargaGrosir = hargaGrosir?.toDouble() ?? 0.0;
                              }

                              var total = jumlah * hargaGrosir;
  ...
  Future<void> _fetchNamaPelanggan() async {
    DocumentSnapshot snapshot =
        await _firestore.collection('namapelanggangrosir').doc('namapelanggan').get();
    setState(() {
  ...
  Future<void> _saveDataToFirestore() async {
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        var hargaGrosir = stockDoc['harga_grosir'];

        await _firestore.collection('transaksigrosir').add({
  ...
  void _calculateTotalBelanjaan(List<DocumentSnapshot> docs) {
    ...
    for (var doc in docs) {
      ...
      var hargaGrosir = doc['harga_grosir'];
      ...
      if (hargaGrosir is double) {
      } else if (hargaGrosir is int) {
        hargaGrosir = hargaGrosir.toDouble();
      } else if (hargaGrosir is String) {
        try {
          hargaGrosir = double.parse(hargaGrosir);
        } catch (e) {
          print('Error parsing hargaGrosir: $e');
